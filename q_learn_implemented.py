...
def current_state():
    ...
    data_state = data_state._append(pd.Series(current_state, index=data_state.columns[:len(current_state)]), ignore_index=True)
...
model.compile(loss=loss_function_used, optimizer=Adam(learning_rate=0.01), metrics=['accuracy'])
...
layer_h1 = [n for n in range(0,64)]
layer_h3 = [n for n in range(0,64)]
